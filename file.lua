...
local author = "Redz"
print("---------------------------------------")
print("SimpleSpy " .. version .. " Loading...")
print("Modified By: " .. author)
print("Status: Initializing UI and Hooks")
print("---------------------------------------")
...
nextb.Parent = SimpleSpy2
nextbCorner.Parent = nextb
...
function mouseMoved()
        if mouseInGui then
                Mouse.Icon = "rbxassetid://6031094672"
        else
                Mouse.Icon = oldIcon or ""
        end
end
function backgroundUserInput(input, gameProcessed)
        if gameProcessed then return end
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
                local mousePos = UserInputService:GetMouseLocation()
                local guiPos = Background.AbsolutePosition
                local guiSize = Background.AbsoluteSize
                mouseInGui = mousePos.X >= guiPos.X and mousePos.X <= guiPos.X + guiSize.X
                        and mousePos.Y >= guiPos.Y - 36 and mousePos.Y <= guiPos.Y + guiSize.Y - 36
        end
end
...
function handlespecials(value, indentation)
        ...
        local maxSize = _G.SimpleSpyMaxStringSize or 10000
        if #value > maxSize then
                reachedMax = true
        end
        return table.concat(buildStr), reachedMax
end
